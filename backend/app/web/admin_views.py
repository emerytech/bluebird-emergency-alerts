...
def _render_sales_inbox_section(messages: object, section: str, unread_count: int = 0) -> str:
    ...
    for msg in (messages or []):
        ...
        rows_html += (
            ...
            f'onclick="bbInboxOpen({msg_id});">View</button> '
            f'<button class="button button-danger-outline" style="font-size:0.72rem;padding:2px 8px;" '
            f'onclick="bbInboxDeleteOne({msg_id});">Delete</button>'
            f'</td>'
            f'</tr>'
        )
    if not rows_html:
    ...
    reply_modal = (
        ...
        '<button type="button" class="button button-danger-outline" style="margin-left:auto;" '
        'onclick="bbInboxDeleteFromModal();">Delete Message</button>'
        '</div>'
        '</form>'
        '</div></div>'
    )
    ...
    inbox_js = (
        ...
        # row click — open if not clicking checkbox
        'function bbInboxRowClick(e,id){'
        ...
        'function bbInboxSelChange(){'
        '  var cbs=document.querySelectorAll(".bb-inbox-cb");'
        '  var checked=Array.from(cbs).filter(function(c){return c.checked;});'
        '  var bar=document.getElementById("bb-inbox-sel-bar");'
        '  var allCb=document.getElementById("bb-inbox-sel-all");'
        '  if(bar){'
        '    bar.style.display=checked.length?"flex":"none";'
        '    var lbl=document.getElementById("bb-inbox-sel-label");'
        ...
        '}'
        'function bbInboxSelectAll(cb){'
        '  document.querySelectorAll(".bb-inbox-cb").forEach(function(c){c.checked=cb.checked;});'
        '  bbInboxSelChange();'
        '}'
        'function bbInboxClearSel(){'
        '  document.querySelectorAll(".bb-inbox-cb").forEach(function(c){c.checked=false;});'
        '  var allCb=document.getElementById("bb-inbox-sel-all");'
        '  if(allCb){allCb.checked=false;allCb.indeterminate=false;}'
        '  bbInboxSelChange();'
        '}'

        # delete one from row button
        'function bbInboxDeleteOne(id){'
        '  if(!confirm("Delete this message? This cannot be undone."))return;'
        '  fetch("/super-admin/inbox/"+id,{method:"DELETE",headers:{"X-Requested-With":"XMLHttpRequest"}})'
        ...
        # delete from modal
        'function bbInboxDeleteFromModal(){'
        '  if(!_bbInboxMsgId)return;'
        '  if(!confirm("Delete this message? This cannot be undone."))return;'
        '  var id=_bbInboxMsgId;'
        '  fetch("/super-admin/inbox/"+id,{method:"DELETE",headers:{"X-Requested-With":"XMLHttpRequest"}})'
        '  .then(r=>r.json()).then(function(d){'
        '    if(d.ok){'
        '      document.getElementById("bb-inbox-modal").style.display="none";'
        '      var row=document.querySelector("tr[data-mid=\'"+id+"\']");'
        '      if(row)row.remove();'
        '      bbInboxSelChange();'
        '    }else alert("Delete failed: "+(d.error||"unknown"));'
        ...
        # bulk delete
        'function bbInboxDeleteSelected(){'
        '  var ids=Array.from(document.querySelectorAll(".bb-inbox-cb:checked")).map(function(c){return parseInt(c.value);});'
        '  if(!ids.length){alert("Nothing selected.");return;}'
        '  if(!confirm("Delete "+ids.length+" message(s)? This cannot be undone."))return;'
        '  fetch("/super-admin/inbox/bulk-delete",{'
        '    method:"POST",'
    ...
    sel_bar = (
        '<div id="bb-inbox-sel-bar" style="display:none;align-items:center;gap:10px;'
        'background:#1e3a5f;border-radius:8px;padding:8px 14px;margin-bottom:10px;flex-wrap:wrap;">'
        '<span id="bb-inbox-sel-label" style="color:#fff;font-size:0.85rem;font-weight:600;">0 selected</span>'
        '<button class="button" style="background:#dc2626;border-color:#dc2626;color:#fff;'
        'font-size:0.8rem;padding:4px 14px;min-height:auto;" onclick="bbInboxDeleteSelected();">'
        '&#128465; Delete Selected</button>'
        '<button class="button button-outline" style="font-size:0.8rem;padding:4px 14px;min-height:auto;color:#fff;border-color:rgba(255,255,255,0.3);" '
        'onclick="bbInboxClearSel();">Clear</button>'
        '</div>'
    )
